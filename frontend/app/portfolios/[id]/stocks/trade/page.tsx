...
export default function TradeStocks() {
    ...
    const fetchCurrentHoldings = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok && data.holdings) {
                const parsedHoldings = data.holdings.map(
                    (holding: HoldingData) => ({
                        ...holding,
    ...
    const fetchCurrentPrice = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `http://localhost:8000/stocks/current-price/${symbol}`
            );
            const data = await res.json();
    ...
    async function handleTransaction(type: 'buy' | 'sell') {
        ...
        if (type === 'buy' && totalCost > portfolioBalance) {
            toast.error(
                `Insufficient funds. Cost: $${totalCost.toFixed(2)}, Balance: $${portfolioBalance.toFixed(2)}`
            );
            return;
        }
        ...
        try {
            const res = await fetch(
                'http://localhost:8000/portfolios/stock-transaction',
                {
                    method: 'POST',
                    headers: {
            ...
            if (res.ok) {
                toast.success(
                    `Successfully ${type === 'buy' ? 'bought' : 'sold'} ${numShares} shares of ${symbol}`
                );
                setShares('');
                fetchPortfolioData();
                fetchCurrentHoldings();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Card>
                    ...
                    <CardContent>
                        <Tabs
                            defaultValue="buy"
                            onValueChange={(v) =>
                                setTransactionType(v as 'buy' | 'sell')
                            }
                        >
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Stocks you currently own in this portfolio
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {currentHoldings.length > 0 ? (
                            ...
                            <div className="text-center py-6 text-muted-foreground">
                                You don&apos;t own any stocks in this portfolio
                                yet.
                            </div>
                        )}
                            ...
                            <div className="mt-6 pt-4 border-t">
                                ...
                                <div className="flex justify-between items-center mt-2">
                                    ...
                                    <span className="font-bold text-lg">
                                        $
                                        {portfolio?.balance
                                            ? Number(portfolio.balance).toFixed(
                                                  2
                                              )
                                            : '0.00'}
                                    </span>
