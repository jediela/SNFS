...
def can_access_list(user_id, list_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT visibility FROM StockLists WHERE list_id = %s", (list_id,))
...
def add_review(user_id, list_id, content):
    ...
    try:
        ...
        if not can_access_list(user_id, list_id):
            return jsonify({"error": "You don't have permission to review this list"}), 403
        ...
        if has_review:
            return jsonify({"error": "You already have a review for this list. Edit your existing review instead."}), 400
...
def update_review(review_id, user_id, content):
    ...
    try:
        ...
        if not owns_review:
            return jsonify({"error": "Review not found or you don't have permission to edit it"}), 403
...
def get_reviews_for_list(list_id, user_id=None):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if not user_id:
                return jsonify({"error": "User ID required to view reviews for non-public lists"}), 400
                
            # Check if user is creator
            is_creator = stock_list["user_id"] == user_id
            
            if not is_creator and not can_access_list(user_id, list_id):
                return jsonify({"error": "You don't have permission to view reviews for this list"}), 403
